# Tasks: MES Middleware Service (Web API Architecture)

**Input**: Design documents from `/specs/002-shared-memory-middleware/`
**Prerequisites**: plan.md (required), spec.md (required), research.md (optional), MIGRATION_GUIDE.md (architecture changes)
...
## ⚠️ ARCHITECTURE MIGRATION STATUS (2025-11-20)

**MIGRATION COMPLETE**: This project has migrated from **Shared Memory IPC** to **Web Service (HTTP REST API)** architecture.

**Key Changes**:
- ❌ **Removed**: SharedMemoryMonitor, SharedMemoryWriter, MiddlewareHostedService, EventWaitHandle synchronization
- ✅ **Added**: InspectionController (HTTP API), StatusController (Monitoring API), Channel<InspectionRecord> (in-memory queue), InspectionChannelProcessor (background service)
- 🔄 **Modified**: Program.cs (WebApplication instead of Host), appsettings.json (Urls, InspectionChannel), IUploadQueueService (new methods)

**Migration Tasks**:
- ✅ T-M001: Create InspectionController with POST /api/inspection/submit endpoint
- ✅ T-M002: Create StatusController with GET /api/status, GET /api/status/history endpoints
- ✅ T-M003: Implement InspectionChannelProcessor to replace SharedMemoryMonitor event handling
- ✅ T-M004: Update Program.cs to WebApplication with Kestrel + Controllers
- ✅ T-M005: Add Channel<InspectionRecord> registration in DI
- ✅ T-M006: Update IUploadQueueService with GetRecentHistoryAsync, RetryUploadAsync
- ✅ T-M007: Update appsettings.json configuration
- ⏳ T-M008: Remove legacy Shared Memory code (files listed in MIGRATION_GUIDE.md)
- ⏳ T-M009: Update WPF Monitor to use HTTP API polling
- ⏳ T-M010: Update integration tests (replace SharedMemoryIpcTests with HTTP controller tests)
- ⏳ T-M011: Update documentation (README.md, equipment integration guides)

**See**: `MIGRATION_GUIDE.md` for complete architecture comparison and deployment steps.
...
- **=� GREEN**: Implement MINIMUM code to make tests pass (no refactoring yet)
